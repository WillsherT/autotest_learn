...
def everything_for_your_cat(cats_data: list) -> str:
    """
    Преобразуем полученный список кортежей содержащих Кличку питомца, возраст питомца, Имя владельца, Фамилию владельца
    в строку формата: Имя и Фамилия владельца: кличка питомца, возраст; ...; кличка питомца, возраст\n
    :param cats_data: список кортежей состоящих из Клички, возраста питомца и Имени, Фамилии владельца.
    :cats_data_dict: Словарь с ключом Имя Фамилия владельца: значением Питомец, возраст;
    :return our_str: Строка формата Имя и Фамилия владельца: питомец, возраст; питомец, возраст; ...; питомец, возраст\n
    """
    cats_data_dict = {}
    for tpl in cats_data:
        owner_name = tpl[2] + ' ' + tpl[3]
        pet_data = tpl[0] + ', ' + str(tpl[1])

        if owner_name in cats_data_dict:
            cats_data_dict[owner_name] = cats_data_dict[owner_name] + '; ' + pet_data
        else:
            cats_data_dict.update({owner_name: pet_data})
